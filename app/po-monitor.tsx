...
function toForm(record: PORecord): POInputFields {
  return {
    previousRevisionId: record.previousRevisionId ? String(record.previousRevisionId) : "", revisionReason: record.revisionReason,
    ...
    budget: record.budget ?? "", contractValue: record.contractValue, scopeType: record.scopeType, baseScopeCommittedValue: record.baseScopeCommittedValue, provisionalScopeCommittedValue: record.provisionalScopeCommittedValue, currencyCode: record.currencyCode, deliveryLeadTimeWeeks: String(record.deliveryLeadTimeWeeks),
...
function asRevision(record: PORecord): POInputFields {
  return { ...toForm(record), previousRevisionId: String(record.id), revisionReason: "", revisionNumber: String(record.revisionNumber + 1) };
}
...
function scopeCommittedTotal(baseValue: string | number, provisionalValue: string | number) {
  const base = Number(baseValue || 0);
  const provisional = Number(provisionalValue || 0);
  return Number.isFinite(base) && Number.isFinite(provisional) ? String(base + provisional) : "";
}
...
function formatEtaToSite(value: string) {
  if (!value) return "—";
...
function downloadTemplate() {
  window.location.assign("/api/pos/template");
}
...
export default function POMonitor({ user, embedded = false }: MonitorProps) {
  ...
  const displayedRecords = useMemo(() => {
    ...
    return source.filter((record) => {
      const matchesSearch = !normalizedQuery || [formatPOReference(record.poNumber, record.projectCode, record.purchasingGroup), record.poNumber, record.location, record.equipmentName, record.vendorName, record.incoterm].join(" ").toLowerCase().includes(normalizedQuery);
  ...
  return <main className={embedded ? "monitor-shell monitor-embedded" : "monitor-shell"}>
    ...
    <section id="dashboard" className="dashboard">
      ...
      <section className="metric-grid" aria-label="PO schedule overview"><Metric title="Active POs" value={metrics.total} detail="Latest revision per PO" accent="blue" /><Metric title="Past ETA to Site" value={metrics.past} detail="Date is already past" accent="orange" /><Metric title="ETA to Site within 30 days" value={metrics.next30} detail="Next delivery window" accent="yellow" /><Metric title="Average lead time" value={`${metrics.averageLeadTime} wk`} detail="Across current revisions" accent="red" /></section>
      <section className="register-card" aria-label="PO register">
        ...
        {loadError ? <div className="error-state" role="alert"><strong>Register unavailable.</strong><p>{loadError}</p><button className="button button-quiet" onClick={() => void refresh()}>Try again</button></div> : loading ? <div className="loading-table" aria-live="polite">Loading the shared register…</div> : displayedRecords.length ? <div className="table-wrap"><table><thead><tr><th>PO / rev.</th><th>Project / vendor</th><th>Equipment</th><th>Group</th><th>PO issued</th><th>Committed value</th><th>Scope split</th><th>ETA to Site</th><th>Bonds</th><th><span className="sr-only">Open details</span></th></tr></thead><tbody>{displayedRecords.map((record) => <tr key={record.id} className={selected?.id === record.id ? "active-row" : ""}><td><strong className="po-number">{formatPOReference(record.poNumber, record.projectCode, record.purchasingGroup)}</strong><span className="subline">Revision {record.revisionNumber}</span></td><td><strong>{record.projectCode || "—"}</strong><span className="subline">{record.vendorName}</span></td><td>{record.equipmentName}</td><td><span className={`group-pill group-${record.purchasingGroup.toLowerCase()}`}>{record.purchasingGroup}</span></td><td>{formatDate(record.releasedDate)}</td><td className="money">{formatMoney(record.contractValue, record.currencyCode)}<span className="subline">{record.currencyCode}</span></td><td className="money"><strong>{record.scopeType}</strong><span className="subline">Base {formatMoney(record.baseScopeCommittedValue, record.currencyCode)} · Prov. {formatMoney(record.provisionalScopeCommittedValue, record.currencyCode)}</span></td><td><span className={`eta-chip eta-${etaKind(record)}`}>{etaLabel(record)}</span><span className="subline">{formatEtaToSite(record.etaRosAtSite)}</span></td><td><BondStatus record={record} /></td><td><button type="button" className="row-action" onClick={() => setSelected(record)} aria-label={`Open ${formatPOReference(record.poNumber, record.projectCode, record.purchasingGroup)} revision ${record.revisionNumber}`}>Open <span aria-hidden="true">→</span></button></td></tr>)}</tbody></table></div> : <EmptyState hasFilters={Boolean(query || group !== "all" || payment !== "all" || project !== "all" || vendor !== "all" || bondFilter !== "all" || eta !== "all")} onNew={() => setEditor({ mode: "create" })} onImport={() => setShowImport(true)} />}
...
function FilterSelect({ label, value, onChange, options, labels = {} }: { label: string; value: string; onChange: (value: string) => void; options: readonly string[]; labels?: Record<string, string> }) { const optionLabel: Record<string, string> = { all: label, required: "Any bond required", notApplicable: "No bond required", past: "Past ETA", next30: "Next 30 days", later: "Beyond 30 days", released: "Latest release", eta: "ETA to Site", po: "PO number", ...labels }; return <label className="select-field"><span className="sr-only">{label}</span><select value={value} onChange={(event) => onChange(event.target.value)}>{options.map((option) => <option key={option} value={option}>{optionLabel[option] ?? option}</option>)}</select></label>; }
...
function DetailDrawer({ record, history, onClose, onEdit, onRevision, onSelectRevision }: { record: PORecord; history: PORecord[]; onClose: () => void; onEdit: () => void; onRevision: () => void; onSelectRevision: (record: PORecord) => void }) {
  const poReference = formatPOReference(record.poNumber, record.projectCode, record.purchasingGroup);
  return <aside className="detail-drawer" role="dialog" aria-modal="true" aria-label={`PO ${poReference} details`}>
    <div className="drawer-top"><div><p className="eyebrow">PO record</p><h2>{poReference}<span>Revision {record.revisionNumber}</span></h2></div><button className="icon-button" onClick={onClose} aria-label="Close PO details">×</button></div>
    ...
    <section className="drawer-section"><div className="drawer-label"><span>Delivery schedule</span><b className={`eta-chip eta-${etaKind(record)}`}>{etaLabel(record)}</b></div><dl className="detail-grid"><Detail label="Incoterm" value={record.incoterm} /><Detail label="Location as per Incoterm" value={record.location} /><Detail label="ETA to Site" value={formatEtaToSite(record.etaRosAtSite)} /><Detail label="Delivery lead time" value={`${record.deliveryLeadTimeWeeks} weeks`} /></dl></section>
    ...
    <section className="drawer-section"><p className="section-title">PO issued & commercial</p><dl className="detail-grid"><Detail label="PO issued date" value={formatDate(record.releasedDate)} /><Detail label="Purchasing group" value={<span className={`group-pill group-${record.purchasingGroup.toLowerCase()}`}>{record.purchasingGroup}</span>} /><Detail label={`Budget (${record.currencyCode})`} value={formatMoney(record.budget, record.currencyCode)} /><Detail label={`Total committed value (${record.currencyCode})`} value={formatMoney(record.contractValue, record.currencyCode)} /><Detail label="Scope type" value={record.scopeType} /><Detail label={`Base scope committed value (${record.currencyCode})`} value={formatMoney(record.baseScopeCommittedValue, record.currencyCode)} /><Detail label={`Provisional scope committed value (${record.currencyCode})`} value={formatMoney(record.provisionalScopeCommittedValue, record.currencyCode)} /><Detail label="Term of payment" value={record.termOfPayment} /></dl></section>
    ...
    <section className="drawer-section history-section"><div className="history-heading"><p className="section-title">Revision history</p><span>{history.length} retained</span></div>{history.map((item) => <button className={item.id === record.id ? "history-row current" : "history-row"} key={item.id} onClick={() => onSelectRevision(item)}><span><b>Revision {item.revisionNumber}</b><small>Released {formatDate(item.releasedDate)}{item.revisionReason ? ` · ${item.revisionReason}` : ""}</small></span><span>{item.id === record.id ? "Viewing" : "Open →"}</span></button>)}</section>
  </aside>;
}
...
function EditorDialog({ state, masters, onClose, onSave }: { state: NonNullable<EditorState>; masters: MasterData; onClose: () => void; onSave: (values: POInputFields, mode: "create" | "edit" | "revision", id?: number) => Promise<void> }) {
  ...
  const calculatedEtaToSite = calculateEtaRosAtSite(values.releasedDate, values.deliveryLeadTimeWeeks, values.location);
  const calculatedContractValue = scopeCommittedTotal(values.baseScopeCommittedValue, values.provisionalScopeCommittedValue);
  ...
  function updateScopeType(scopeType: string) { setValues((current) => ({ ...current, scopeType, baseScopeCommittedValue: scopeType === "Provisional scope" ? "0" : current.baseScopeCommittedValue, provisionalScopeCommittedValue: scopeType === "Base scope" ? "0" : current.provisionalScopeCommittedValue })); }
  function updateScopeValue(key: "baseScopeCommittedValue" | "provisionalScopeCommittedValue", value: string) { setValues((current) => ({ ...current, [key]: value, contractValue: scopeCommittedTotal(key === "baseScopeCommittedValue" ? value : current.baseScopeCommittedValue, key === "provisionalScopeCommittedValue" ? value : current.provisionalScopeCommittedValue) })); }
  ...
  return <div className="modal-backdrop" role="presentation"><section className="form-dialog" role="dialog" aria-modal="true" aria-labelledby="editor-title">
    ...
    <form onSubmit={submit}>
      <fieldset><legend>Identity & PO issued</legend><div className="form-grid"><Field label="PO No." value={values.poNumber} onChange={(value) => update("poNumber", value)} placeholder="321000XXXX" disabled={isEdit} required /><Field label="Revision number" type="number" min="0" value={values.revisionNumber} onChange={(value) => update("revisionNumber", value)} disabled={isEdit} required /><Field label="PO issued date" type="date" value={values.releasedDate} onChange={(value) => update("releasedDate", value)} required /><SelectField label="Purchasing group" value={values.purchasingGroup} onChange={(value) => update("purchasingGroup", value)} options={purchasingGroups} /><MasterSelect label="Project code" value={String(values.projectId)} onChange={(value) => update("projectId", value)} options={masters.projects} optionLabel={(project) => `${project.project_code}${project.project_name ? ` — ${project.project_name}` : ""}`} /><SearchableSelect label="Vendor" value={String(values.vendorId)} onChange={updateVendor} required placeholder="Search vendor…" emptyLabel={masters.vendors.length ? "No matching vendor" : "No active vendor master data"} options={masters.vendors.filter((vendor) => vendor.is_active || String(vendor.id) === String(values.vendorId)).map((vendor) => ({ value: String(vendor.id), label: `${vendor.vendor_name} (${vendor.vendor_code})${vendor.is_active ? "" : " (inactive)"}` }))} /><Field label="Equipment name" value={values.equipmentName} onChange={(value) => update("equipmentName", value)} required /></div></fieldset>
      {state.mode === "revision" && <fieldset><legend>Revision control</legend><label className="textarea-field"><span>Revision reason</span><textarea value={values.revisionReason} onChange={(event) => update("revisionReason", event.target.value)} rows={3} maxLength={1000} required placeholder="Describe the commercial or scope change in this revision" /></label></fieldset>}
      <fieldset><legend>Commercial terms</legend><p className="service-help">Separate provisional value because it will be backcharged to the client. Total committed value is calculated from the two scope values. Budget headroom uses base scope only.</p><div className="form-grid"><Field label={`Budget (${values.currencyCode}) — optional`} type="number" min="0" step="0.01" value={values.budget} onChange={(value) => update("budget", value)} placeholder="Leave blank when unavailable" /><SelectField label="Scope type" value={values.scopeType} onChange={updateScopeType} options={scopeTypes} /><Field label={`Base scope committed value (${values.currencyCode})`} type="number" min="0" step="0.01" value={values.baseScopeCommittedValue} onChange={(value) => updateScopeValue("baseScopeCommittedValue", value)} disabled={values.scopeType === "Provisional scope"} required /><Field label={`Provisional scope committed value (${values.currencyCode})`} type="number" min="0" step="0.01" value={values.provisionalScopeCommittedValue} onChange={(value) => updateScopeValue("provisionalScopeCommittedValue", value)} disabled={values.scopeType === "Base scope"} required /><Field label={`Total committed value (${values.currencyCode})`} value={calculatedContractValue} onChange={() => undefined} disabled /><SelectField label="Currency" value={values.currencyCode} onChange={(value) => update("currencyCode", value)} options={currencyCodes} /><SelectField label="Term of payment" value={values.termOfPayment} onChange={(value) => update("termOfPayment", value)} options={paymentTerms} /></div></fieldset>
      <fieldset><legend>Delivery schedule</legend><p className="service-help">ETA to Site is calculated from PO issued date + delivery lead time + transit to site: Jakarta 2 weeks, Overseas 3 weeks, Site 0 weeks.</p><div className="form-grid"><IncotermSelect value={values.incoterm} onChange={(value) => update("incoterm", value)} /><IncotermLocationSelect value={values.location} onChange={(value) => update("location", value)} /><Field label="Delivery lead time (weeks)" type="number" min="0" step="1" value={values.deliveryLeadTimeWeeks} onChange={(value) => update("deliveryLeadTimeWeeks", value)} required /><CalculatedEtaField value={calculatedEtaToSite} /></div></fieldset>
...
function IncotermLocationSelect({ value, onChange }: { value: string; onChange: (value: string) => void }) { const selected = (incotermLocations as readonly string[]).includes(value); return <label className="form-field"><span>Location as per Incoterm</span><select value={selected ? value : ""} onChange={(event) => onChange(event.target.value)} required><option value="" disabled>Select location</option>{incotermLocations.map((location) => <option key={location} value={location}>{location}</option>)}</select></label>; }
function CalculatedEtaField({ value }: { value: string }) { return <label className="form-field"><span>ETA to Site (calculated)</span><input value={formatEtaToSite(value)} readOnly placeholder="Complete PO issued date, lead time, and location" /></label>; }
...
function ImportDialog({ onClose, onImported }: { onClose: () => void; onImported: (count: number) => Promise<void> }) {
  ...
  function chooseFile(next: File | null) { setFile(next); setError(""); setRowErrors([]); setPreview(next ? "The workbook will be checked before any PO record is added." : ""); }
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!file) { setError("Choose an Excel template to import."); return; } setImporting(true); setError(""); setRowErrors([]); try { const formData = new FormData(); formData.append("file", file); const response = await fetch("/api/pos/import", { method: "POST", body: formData }); const payload = (await response.json()) as { imported?: number; error?: string; errors?: string[] }; if (!response.ok) { setError(payload.error ?? "Unable to import the Excel file."); setRowErrors(payload.errors ?? []); return; } await onImported(payload.imported ?? 0); } catch { setError("Unable to import the Excel file. Please try again."); } finally { setImporting(false); } }
  return <div className="modal-backdrop" role="presentation"><section className="import-dialog" role="dialog" aria-modal="true" aria-labelledby="import-title"><div className="dialog-top"><div><p className="eyebrow">Bulk intake</p><h2 id="import-title">Import PO register</h2><p>Use the Excel template so data types and dropdown values stay consistent with the PO register.</p></div><button className="icon-button" type="button" onClick={onClose} aria-label="Close import">×</button></div><ol className="import-rules"><li>Download and use the <b>.xlsx</b> PO Register template. Do not rename or rearrange its headers.</li><li>Enter PO issued date as an Excel date displayed <b>YYYY-MM-DD</b>; currency, lead time, and service values use typed number cells.</li><li>Use the provided dropdowns for purchasing group, location, currency, Incoterm, payment term, PB/WB, and service inclusion.</li><li>Project code is optional, but any entered project code and vendor name must already exist in Master Data.</li><li>For PB/WB set to Yes, enter the validity date; use <b>N/A</b> when No. ETA to Site is calculated after import.</li></ol><button type="button" className="template-link" onClick={downloadTemplate}>↓ Download Excel template</button><form onSubmit={submit}><input ref={fileRef} className="file-input" type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={(event) => chooseFile(event.target.files?.[0] ?? null)} /><div className="file-choice"><span>{file ? file.name : "No Excel file selected"}</span><button type="button" className="button button-quiet" onClick={() => fileRef.current?.click()}>Choose file</button></div>{preview && <p className="preview-note">{preview}</p>}{error && <p className="form-error" role="alert">{error}</p>}{rowErrors.length > 0 && <ul className="import-errors">{rowErrors.slice(0, 8).map((message, index) => <li key={`${message}-${index}`}>{message}</li>)}{rowErrors.length > 8 && <li>…and {rowErrors.length - 8} more rows to fix.</li>}</ul>}<div className="form-actions"><button type="button" className="button button-quiet" onClick={onClose}>Cancel</button><button type="submit" className="button button-primary" disabled={importing}>{importing ? "Checking & importing…" : "Confirm import"}</button></div></form></section></div>;
}
