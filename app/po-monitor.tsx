...
function EditorDialog({ state, masters, onClose, onSave }: { state: NonNullable<EditorState>; masters: MasterData; onClose: () => void; onSave: (values: POInputFields, mode: "create" | "edit" | "revision", id?: number) => Promise<void> }) {
  ...
  return <div className="modal-backdrop" role="presentation"><section className="form-dialog" role="dialog" aria-modal="true" aria-labelledby="editor-title">
    ...
    <form onSubmit={submit}>
      ...
      <fieldset><legend>Commercial terms</legend><p className="service-help">Budget is optional. Leave it blank when the project has no approved budget; the PO remains monitored but is excluded from Budget Headroom.</p><div className="form-grid"><Field label={`Budget (${values.currencyCode}) — optional`} type="number" min="0" step="0.01" value={values.budget} onChange={(value) => update("budget", value)} placeholder="Leave blank when unavailable" /><Field label={`Contract value (${values.currencyCode})`} type="number" min="0" step="0.01" value={values.contractValue} onChange={(value) => update("contractValue", value)} required /><SelectField label="Currency" value={values.currencyCode} onChange={(value) => update("currencyCode", value)} options={currencyCodes} /><SelectField label="Term of payment" value={values.termOfPayment} onChange={(value) => update("termOfPayment", value)} options={paymentTerms} /></div></fieldset>
