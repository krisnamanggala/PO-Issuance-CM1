...
export default function ExecutionBoard() {
  ...
  const deliveryRisks = latest.filter(({ po, update }) => {
    const forecast = update?.forecastEtaSite ?? po.etaRosAtSite;
  ...
  return <>
    ...
    <section className="dashboard-kpis execution-kpis" aria-label="Delivery and cash indicators">
      <article className="dashboard-kpi critical"><span>Past delivery forecast at site</span><strong>{deliveryRisks.length}</strong><small>Open POs past latest forecast or contract ETA site</small></article>
    ...
    <section className="panel"><div className="panel-heading"><div><h2>Latest delivery position</h2><p>One latest execution update per current PO revision; the contractual ETA site remains visible.</p></div></div>{loading ? <div className="loading-table">Loading delivery updates…</div> : latest.length ? <div className="table-scroll"><table className="action-table"><thead><tr><th>PO / vendor</th><th>Contract ETA Site</th><th>Forecast ETA Site</th><th>Status</th><th>Actual ROS</th><th>Last update</th></tr></thead><tbody>{latest.map(({ po, update }) => <tr key={po.id}><td><strong>{po.projectCode ? `${po.projectCode}-` : ""}{po.purchasingGroup}-{po.poNumber} Rev.{po.revisionNumber}</strong><span>{po.vendorName}</span></td><td>{date(po.etaRosAtSite)}</td><td>{date(update?.forecastEtaSite ?? null)}</td><td><span className={`status-badge status-${update?.deliveryStatus ?? "n/a"}`}>{update?.deliveryStatus?.replaceAll("-", " ") ?? "No update"}</span></td><td>{date(update?.actualRosDate ?? null)}</td><td>{date(update?.updateDate ?? null)}</td></tr>)}</tbody></table></div> : <Empty title="No PO execution records yet." copy="Create a PO first, then add delivery updates without changing its contractual schedule." />}</section>
...
function POSelect({ pos }: { pos: PORecord[] }) { return <label className="form-field delivery-po-select"><span>PO No.</span><select name="poRevisionId" required title="Choose current PO revision"><option value="">Choose current PO revision</option>{pos.map((po) => <option key={po.id} value={po.id}>{po.poNumber} · Rev. {po.revisionNumber} · {po.vendorName}</option>)}</select></label>; }
