...
const headers = ["po_number", "revision_number", "bond_type", "bond_number", "issuing_bank", "currency_code", "bond_value", "expected_value", "received_date", "issue_date", "effective_date", "expiry_date", "released_date", "remarks"] as const;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const entries = parsed.rows.map((row, index) => {
      ...
      const input = { poRevisionId: poRevisionId ?? "", bondType: row.bond_type, bondNumber: row.bond_number, issuingBank: row.issuing_bank, currencyCode: row.currency_code || "IDR", bondValue: row.bond_value, expectedValue: row.expected_value, receivedDate: row.received_date, issueDate: row.issue_date, effectiveDate: row.effective_date, expiryDate: row.expiry_date, releasedDate: row.released_date, remarks: row.remarks };
