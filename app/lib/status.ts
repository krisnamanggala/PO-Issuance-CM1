...
export function deliveryStatus(record: PORecord, settings = defaultDashboardSettings, today = new Date()): DeliveryStatus {
  if (record.cancelledAt) return "cancelled";
  if (record.deliveryCompletedAt) return "completed";
  const days = daysUntil(record.etaRosAtSite, today);
...
export function isActivePO(record: PORecord) {
  return !record.deliveryCompletedAt && !record.cancelledAt;
}
...
export function criticalActions(records: PORecord[], bonds: BondRecord[], settings = defaultDashboardSettings, today = new Date()) {
  const actions: CriticalAction[] = [];
  for (const record of latestRevisions(records)) {
    if (!isActivePO(record)) continue;
    const delivery = deliveryStatus(record, settings, today);
    const etaDays = daysUntil(record.etaRosAtSite, today);
    const common = {
    ...
    if (delivery === "delayed" || delivery === "due-soon" || delivery === "missing-eta") {
      actions.push({
        ...
        issueType: delivery === "delayed" ? "PO delivery overdue" : delivery === "due-soon" ? "PO delivery due within 30 days" : "PO missing ETA",
        relevantDate: delivery === "missing-eta" ? null : record.etaRosAtSite,
...
export function dashboardMetrics(records: PORecord[], bonds: BondRecord[], settings = defaultDashboardSettings, today = new Date()) {
  const current = latestRevisions(records);
  const active = current.filter(isActivePO);
  const delivery = active.map((record) => deliveryStatus(record, settings, today));
  ...
  return {
    ...
    deliveryBreakdown: Object.fromEntries(["on-track", "due-soon", "delayed", "completed", "missing-eta"].map((status) => [status, current.filter((record) => deliveryStatus(record, settings, today) === status).length])),
...
export function dashboardVisuals(records: PORecord[], bonds: BondRecord[], settings = defaultDashboardSettings, today = new Date()): DashboardVisuals {
  ...
  for (const record of latestRevisions(records)) {
    ...
    if (delivery === "delayed") {
      ...
      const overdueDays = Math.abs(daysUntil(record.etaRosAtSite, today) ?? 0);
      vendor.count += 1;
